import assert from "node:assert/strict";
import { describe, it } from "node:test";
import { encodeFunctionData, parseEther, createWalletClient, createPublicClient, http } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { arbitrumSepolia } from "viem/chains";

describe("EIP-7702 Real Delegation Test", async function () {
  const RPC_URL = "https://sepolia-rollup.arbitrum.io/rpc";
  
  // Contract addresses from deployment
  const SMART_WALLET_ADDRESS = "0x188CB9276bb75992A6c1Af2443e293431307382a";
  const PYUSD_ADDRESS = "0x637A1259C6afd7E3AdF63993cA7E58BB438aB1B1";
  const EOA_DELEGATION_ADDRESS = "0x67e8b159534f197cfd368210db6f48a59422ff18";
  
  // User account (Hardhat account #1)
  const userPrivateKey = "0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d";
  const userAddress = "0x70997970c51812dc3a010c7d01b50e0d17dc79c8";
  
  // Paymaster account (your provided key)
  const paymasterPrivateKey = "0x3471db0b3f6db7801ab954a3ba20596a99ae8d4da0b5c8dd1228a118dea3d1a6";
  const paymasterAddress = "0x53cd866553b78a32060b70e764d31b0fe3afe52c";
  
  let userWallet: any;
  let paymasterWallet: any;
  let publicClient: any;
  
  it("Setup wallet clients", async function () {
    // Create user wallet client
    const userAccount = privateKeyToAccount(userPrivateKey as `0x${string}`);
    userWallet = createWalletClient({
      account: userAccount,
      chain: arbitrumSepolia,
      transport: http(RPC_URL)
    });
    
    // Create paymaster wallet client
    const paymasterAccount = privateKeyToAccount(paymasterPrivateKey as `0x${string}`);
    paymasterWallet = createWalletClient({
      account: paymasterAccount,
      chain: arbitrumSepolia,
      transport: http(RPC_URL)
    });
    
    // Create public client
    publicClient = createPublicClient({
      chain: arbitrumSepolia,
      transport: http(RPC_URL)
    });
    
    console.log("✅ Wallet clients created");
    console.log("   User EOA:", userAddress);
    console.log("   Paymaster EOA:", paymasterAddress);
    console.log("   SmartWallet:", SMART_WALLET_ADDRESS);
  });

  it("Check SmartWallet PYUSD balance", async function () {
    // Get PYUSD contract
    const pyusdAbi = [
      {
        "inputs": [{"name": "account", "type": "address"}],
        "name": "balanceOf",
        "outputs": [{"name": "", "type": "uint256"}],
        "stateMutability": "view",
        "type": "function"
      }
    ];
    
    const balance = await publicClient.readContract({
      address: PYUSD_ADDRESS as `0x${string}`,
      abi: pyusdAbi,
      functionName: "balanceOf",
      args: [SMART_WALLET_ADDRESS]
    });
    
    console.log("💰 SmartWallet PYUSD balance:", (balance / 10n**6n).toString(), "PYUSD");
    assert(balance > 0n, "SmartWallet should have PYUSD balance");
  });

  it("Transfer 1 PYUSD using EIP-7702 delegation", async function () {
    const recipient = paymasterAddress; // Send to paymaster
    const transferAmount = 1n * 10n**6n; // 1 PYUSD (6 decimals)
    
    console.log("🚀 Starting EIP-7702 transfer...");
    console.log("   From: SmartWallet", SMART_WALLET_ADDRESS);
    console.log("   To: Paymaster", recipient);
    console.log("   Amount: 1 PYUSD");
    
    // Step 1: User signs EIP-7702 authorization
    console.log("🔐 User signing EIP-7702 authorization...");
    const authorization = await userWallet.signAuthorization({
      contractAddress: SMART_WALLET_ADDRESS as `0x${string}`,
      chainId: 421614, // Arbitrum Sepolia
      nonce: 0
    });
    
    console.log("✅ User signed EIP-7702 authorization");
    console.log("   Delegating EOA to SmartWallet");
    
    // Step 2: Prepare PYUSD transfer data
    const transferData = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "to", "type": "address"},
            {"name": "amount", "type": "uint256"}
          ],
          "name": "transfer",
          "outputs": [{"name": "", "type": "bool"}],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "transfer",
      args: [recipient, transferAmount]
    });
    
    // Step 3: Prepare SmartWallet.execute() call
    const executeData = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "target", "type": "address"},
            {"name": "value", "type": "uint256"},
            {"name": "data", "type": "bytes"}
          ],
          "name": "execute",
          "outputs": [],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "execute",
      args: [PYUSD_ADDRESS, 0n, transferData]
    });
    
    // Step 4: Paymaster submits transaction with authorization
    console.log("💸 Paymaster submitting transaction with EIP-7702 authorization...");
    const hash = await paymasterWallet.sendTransaction({
      to: SMART_WALLET_ADDRESS as `0x${string}`,
      data: executeData,
      authorizationList: [authorization]
    });
    
    console.log("✅ Paymaster submitted transaction:", hash);
    
    // Wait for transaction
    console.log("⏳ Waiting for transaction confirmation...");
    const receipt = await publicClient.waitForTransactionReceipt({ hash });
    console.log("✅ Transaction confirmed in block:", receipt.blockNumber);
    
    // Verify transfer
    console.log("🔍 Verifying transfer...");
    const recipientBalance = await publicClient.readContract({
      address: PYUSD_ADDRESS as `0x${string}`,
      abi: [
        {
          "inputs": [{"name": "account", "type": "address"}],
          "name": "balanceOf",
          "outputs": [{"name": "", "type": "uint256"}],
          "stateMutability": "view",
          "type": "function"
        }
      ],
      functionName: "balanceOf",
      args: [recipient]
    });
    
    const smartWalletBalance = await publicClient.readContract({
      address: PYUSD_ADDRESS as `0x${string}`,
      abi: [
        {
          "inputs": [{"name": "account", "type": "address"}],
          "name": "balanceOf",
          "outputs": [{"name": "", "type": "uint256"}],
          "stateMutability": "view",
          "type": "function"
        }
      ],
      functionName: "balanceOf",
      args: [SMART_WALLET_ADDRESS]
    });
    
    console.log("🎉 Gasless transfer successful!");
    console.log("   Gas paid by: Paymaster");
    console.log("   Transaction executed as: User EOA");
    console.log("   Recipient balance:", (recipientBalance / 10n**6n).toString(), "PYUSD");
    console.log("   SmartWallet balance:", (smartWalletBalance / 10n**6n).toString(), "PYUSD");
    
    // Verify the transfer worked
    assert(recipientBalance >= transferAmount, "Recipient should have received PYUSD");
    console.log("✅ Transfer verification passed!");
  });

  it("Multiple transactions with incrementing nonce", async function () {
    const recipient = paymasterAddress;
    const transferAmount = 1n * 10n**6n; // 1 PYUSD
    
    console.log("🔄 Testing multiple transactions with nonce increment...");
    
    // Transaction 1 with nonce 1
    console.log("📝 Transaction 1 (nonce 1)...");
    const auth1 = await userWallet.signAuthorization({
      contractAddress: SMART_WALLET_ADDRESS as `0x${string}`,
      chainId: 421614,
      nonce: 1
    });
    
    const transferData1 = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "to", "type": "address"},
            {"name": "amount", "type": "uint256"}
          ],
          "name": "transfer",
          "outputs": [{"name": "", "type": "bool"}],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "transfer",
      args: [recipient, transferAmount]
    });
    
    const executeData1 = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "target", "type": "address"},
            {"name": "value", "type": "uint256"},
            {"name": "data", "type": "bytes"}
          ],
          "name": "execute",
          "outputs": [],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "execute",
      args: [PYUSD_ADDRESS, 0n, transferData1]
    });
    
    const hash1 = await paymasterWallet.sendTransaction({
      to: SMART_WALLET_ADDRESS as `0x${string}`,
      data: executeData1,
      authorizationList: [auth1]
    });
    
    await publicClient.waitForTransactionReceipt({ hash: hash1 });
    console.log("✅ Transaction 1 completed:", hash1);
    
    // Transaction 2 with nonce 2
    console.log("📝 Transaction 2 (nonce 2)...");
    const auth2 = await userWallet.signAuthorization({
      contractAddress: SMART_WALLET_ADDRESS as `0x${string}`,
      chainId: 421614,
      nonce: 2
    });
    
    const transferData2 = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "to", "type": "address"},
            {"name": "amount", "type": "uint256"}
          ],
          "name": "transfer",
          "outputs": [{"name": "", "type": "bool"}],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "transfer",
      args: [recipient, transferAmount]
    });
    
    const executeData2 = encodeFunctionData({
      abi: [
        {
          "inputs": [
            {"name": "target", "type": "address"},
            {"name": "value", "type": "uint256"},
            {"name": "data", "type": "bytes"}
          ],
          "name": "execute",
          "outputs": [],
          "stateMutability": "nonpayable",
          "type": "function"
        }
      ],
      functionName: "execute",
      args: [PYUSD_ADDRESS, 0n, transferData2]
    });
    
    const hash2 = await paymasterWallet.sendTransaction({
      to: SMART_WALLET_ADDRESS as `0x${string}`,
      data: executeData2,
      authorizationList: [auth2]
    });
    
    await publicClient.waitForTransactionReceipt({ hash: hash2 });
    console.log("✅ Transaction 2 completed:", hash2);
    
    console.log("🎉 Multiple gasless transactions successful!");
    console.log("   Both transactions paid by: Paymaster");
    console.log("   Both transactions executed as: User EOA");
  });
});